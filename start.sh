...
command -v bun >/dev/null 2>&1 || { echo "Error: bun is not installed. Get it at https://bun.sh/"; exit 1; }

ROOT="$(cd "$(dirname "$0")" && pwd)"

echo "Installing dependencies..."
(cd "$ROOT/backend" && dotnet restore --verbosity quiet)
(cd "$ROOT/frontend" && bun install --silent)
...
(cd "$ROOT/frontend" && bun run dev --open http://scdigger.localhost:5173) &
